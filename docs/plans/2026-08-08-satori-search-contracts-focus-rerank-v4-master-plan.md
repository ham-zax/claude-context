...
### Task 14 — Add LateOn Context-v4 Profile and Production Integration — DONE (7f9aa81)

- Added: `runtime-profile-v4-d32.json` (`lateon_offline_quality_projection_v4_d32_v1`, schema `satori_lateon_runtime_profile_v4`, qualification `owner_activated_operationally_qualified_not_held_out`, identity `search_rerank_document_v4` + `search_rerank_query_v2`, projectionSha256 `de52c67d…` = SHA-256 of `search-rerank-document-v4.ts` per the O0 receipt convention; artifacts/runtime/inference/execution/operational bounds byte-identical to the v3 activated profile) + acquisition manifest (profile digest `250d57c4…`).
- Protocol: `contextV4D32` profile id, `ownerDefaultContextV4` policy id, `LateOnRuntimeProfileV4` type; the reranker loader validates the v4 schema and advertises `search_rerank_query_v2`/`search_rerank_document_v4`; mcp config pairing matrix enforces v4-policy ↔ v4-profile (v3 profiles get the `satori upgrade` migration hint; other profiles fail plainly). Compatibility: v1/v2 → raw query + historical docs; v3 → query-v1 + document-v3; v4 → query-v2 + document-v4.
- CLI: defaults flipped to the v4 profile + `lateon_context_v4_d32_owner_default_v1` (frozen digest `250d57c4…`, v4 asset filenames); the previous managed default (v3-d32-v2 + v3 policy) is admitted as a migratable combination and `satori upgrade` rewrites it atomically (probe failure leaves the launcher byte-identical — new regression test covers both halves); doctor flags the previous v3 policy with upgrade guidance; release-smoke asserts the new frozen identity.
- Verification: RED proved (loader rejected the v4 schema; pairing matrix missing; migration target stale); GREEN — 2 new mcp profile/pairing tests + 1 new atomic-migration test + 1 new reranker identity test; mcp suite 1481/1481, cli suite 342/342 (was 341), both release-smoke suites, contract:check (d5aa4a07…), manifest:check, docs:check, typechecks all green.

### Task 15 — Restore Complete Verification and Seal the Rollout — DONE (2705b9b)

- 15.1 fixed: `fetch-with-deadline.test.ts` now injects a deterministic retryable failure (stub fetch rejecting with a listed `ECONNREFUSED` code) instead of relying on environment-dependent real-connection classification — 16/16 green.
- 15.2 fixed: stale eval pins regenerated only after `validateTaskKey` verified current source anchors — `known-exact-target` 189–604 → 188–606 (shifted by Task 5), `unknown-freshness-reuse` owner 229–392 → 238–430 and relation 135–138 → 139–142 (shifted by Task 8); all task keys re-validated (0 stale).
- 15.3 mojibake: byte scan confirms no committed corruption (only intentional pattern quotes).
