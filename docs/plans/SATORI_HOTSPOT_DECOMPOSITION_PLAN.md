...
- Phase 9.3 precondition preflight (read-only): V5-only writer is NOT representable by all supported production publication paths. `publishSealedPolicyBindingForMarker` (4 call sites in index-generation-workflow.ts) constructs bindings without `publication`, falling to V3. Navigation repair (`repairPolicy` from `getPublishedResolvedPolicy`) may lack `controlSignature`, falling to V4. Full-index activation (Path 1) and incremental reconciliation (Path 4) always supply both values and produce V5. The fix for Path 2 is to carry forward the existing binding's `publication`; the current binding is already read at the same call site. The fix for Path 3 is similar — inherit `controlSignature` from the stored policy. `loadCustomIndexPolicy` (reader) must return `requires_reindex` for V3/V4 once V5 is canonical. `restoreDurableIndexAuthority` writes raw bytes and is version-preserving. No production commit.

Next open batch at this checkpoint: Phase 9.3 precondition preflight complete.
The durable-format floor (9.3) requires separate authorization with the verbatim
text specified in the batch contract. Phase 8 is sealed (`bc53cb4 → 0963ca9`,
review-approved). Refresh this checkpoint only after an accepted batch is
