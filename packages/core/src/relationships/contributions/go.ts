...
function findEnclosingCaller(
    ...
    for (const sym of fileSymbols) {
        ...
        // Check if call is within symbol span
        const contains = (
...
export interface GoResolutionInput extends CallResolutionEngineInput {
...
export class GoResolutionContributionEngine implements CallResolutionEngine {
    resolveCalls(input: CallResolutionEngineInput & { semanticEvidence?: SemanticProjectEvidence }): CallResolutionContribution {
        ...
        if (!semanticEvidence || semanticEvidence.language !== 'go') {
        ...
        for (const [filePath, occurrences] of semanticEvidence.occurrencesByFile) {
            ...
            for (const occ of occurrences) {
                ...
                const claim: ResolutionClaim = {
                    providerId: CBM_GO_PROVIDER_ID,
                    providerVersion: CBM_GO_PROVIDER_VERSION,
                    environmentConfigId: CBM_GO_ENVIRONMENT_CONFIG_ID,
                    sourceFile: filePath,
                ...
                if (decision === 'resolved' && targetSymbol) {
                    const isTest = isTestOrFixturePath(filePath);
                    const relationshipType = isTest ? 'TESTS' : 'CALLS';
                    ...
                    // If test path, also record the CALLS edge
                    if (isTest) {
                        records.push({
                            sourceKey: caller.symbolKey,
                            sourceInstanceId: caller.symbolInstanceId,
