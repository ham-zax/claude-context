...
test("runDoctor surfaces the installer-bound LateOn activation policy", async () => {
    ...
    try {
        ...
        assert.equal(policy?.message, "LateOn activation policy: lateon_context_v3_d32_owner_default_v1.");
        assert.equal(
...
test("runDoctor flags a managed launcher whose LateOn activation policy contradicts its profile", async () => {
    ...
    try {
        ...
        assert.match(
            policy?.message || "",
            /requires SATORI_LATEON_PROFILE=lateon_offline_quality_projection_v3_d32_v2; received lateon_projection_v2_d16_v1/,
        );
    } finally {
        fs.rmSync(tempDir, { recursive: true, force: true });
    }
