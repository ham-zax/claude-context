...
export const callGraphTool: McpTool = {
    name: 'call_graph',
    description: () => 'Traverse conservative relationship navigation for a graph-ready canonical symbol. Pass a grouped search result target as symbolRef and its codebaseRoot as path. CALLS v0 support is language/capability-specific; traversals are bounded and incomplete and may include resolved testReferences where supported. Treat callers/callees as navigation evidence, not blast-radius proof, and do not treat empty results as proof of no callers. Verify important inbound impact with search_codebase, read_file, tests, or direct references.',
