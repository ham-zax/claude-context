...
const buildSearchSchema = (ctx: ToolContext) => z.object({
    ...
    limit: z.number().int().positive().max(ctx.capabilities.getMaxSearchResultTotal()).default(ctx.capabilities.getDefaultSearchLimit()).optional().describe("Total caller bound across all pages: frozen groups in grouped mode or returned chunks in raw mode. It is not the initial page size."),
    disclosureLimit: z.number().int().positive().max(ctx.capabilities.getMaxSearchPageSize()).optional().describe("Initial grouped-result page size only. Grouped searches show at most 10 results initially when omitted. For example, limit=20 and disclosureLimit=6 returns 6 initially and freezes up to 20 total for continuation. Retrieval depth and reranker admission are independent."),
