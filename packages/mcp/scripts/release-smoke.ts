...
function runPackedCoreParserSmoke(runtimeRoot: string): void {
    const script = [
        ...
        "const exactSlice = (source, symbol) => Buffer.from(source).subarray(symbol.span.startByte, symbol.span.endByte).toString('utf8');",
        "Promise.all([",
        "  analyzer.analyze({ content: tsSource, language: 'typescript', relativePath: 'src/run.ts' }),",
        "  analyzer.analyze({ content: pySource, language: 'python', relativePath: 'src/service.py' }),",
        "  analyzer.analyze({ content: scalaSource, language: 'scala', relativePath: 'src/Service.scala' }),",
        "]).then(([typescript, python, scala]) => {",
        "  if (typescript.backend !== 'oxc' || typescript.structuralStatus !== 'complete') process.exit(2);",
        "  if (exactSlice(tsSource, typescript.symbols.find((symbol) => symbol.name === 'run')) !== 'function run() { return \"你好😀\"; }') process.exit(3);",
        "  if (python.backend !== 'tree_sitter_wasm' || python.structuralStatus !== 'complete') process.exit(3);",
        "  if (exactSlice(pySource, python.symbols.find((symbol) => symbol.name === 'run')) !== 'def run():\\n    return \"你好\"') process.exit(4);",
